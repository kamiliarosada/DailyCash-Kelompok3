...
class CashViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun getExpenseByDateRange(userId: String, startDate: Long, endDate: Long) =
        repository.getExpenseByDateRange(userId, startDate, endDate)

    fun getAllFixedExpenses(userId: String) = repository.getAllFixedExpenses(userId)
    fun insertFixedExpense(fixedExpense: FixedExpenseEntity) = viewModelScope.launch { repository.insertFixedExpense(fixedExpense) }
    fun updateFixedExpense(fixedExpense: FixedExpenseEntity) = viewModelScope.launch { repository.updateFixedExpense(fixedExpense) }
    fun deleteFixedExpense(fixedExpense: FixedExpenseEntity) = viewModelScope.launch { repository.deleteFixedExpense(fixedExpense) }
    ...
    fun insertOrUpdateBudget(budget: BudgetEntity) = viewModelScope.launch { repository.insertOrUpdateBudget(budget) }
